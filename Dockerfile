...
ENV INSTALL_PATH /eureka
# The name of the application is eureka and while there
...
# having to CD into the /eureka folder every time would be
# lame, so instead we can set the WORKDIR to be /eureka.
#
...
# eureka/ path inside of the Docker image.
#
# It copies it to /eureka because of the WORKDIR being set.
#
...
# /eureka folder inside of the Docker image.
#
